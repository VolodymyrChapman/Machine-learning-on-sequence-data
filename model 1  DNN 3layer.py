...
model1 = tf.keras.models.Sequential([
    tf.keras.layers.Dense(350, activation = 'relu', input_dim = 1467),
...
history1 = model1.fit(X_train, y_train_binary, epochs = 10, validation_data= (X_CV, y_CV_binary))

#evaluate on test data
test_loss = model1.evaluate(X_test, y_test_binary)
...
#experimenting with precision and recall
...
# compile the model
model1.compile(optimizer='adam', loss='categorical_crossentropy', metrics=['acc',f1_m,precision_m, recall_m])

# fit the model
history1 = model1.fit(X_train, y_train_binary, validation_data= (X_CV, y_CV_binary), epochs=10) #, verbose=0

# evaluate the model
loss, accuracy, f1_score, precision, recall = model1.evaluate(X_test, y_test_binary, verbose=0)
